...
if __name__ == '__main__':
    ...
    if os.path.exists(os.path.join(sc2path, 'Maps/Ladder2019Season2')):
    ...
    if args.model1 is not None:
        model1 = os.path.join(os.path.join(os.path.dirname(__file__), args.model1 + '.pth'))
    ...
    if args.model2 is not None:
        model2 = os.path.join(os.path.join(os.path.dirname(__file__), args.model2 + '.pth'))
    ...
    if args.game_type == 'agent_vs_agent':
        user_config.env.player_ids = [os.path.basename(model1).split('.')[0], os.path.basename(model2).split('.')[0]]
        ...
        user_config.env.player_ids = [os.path.basename(model1).split('.')[0], bot_level]
